...
{
       0,    42,    42,    43,    44,    45,    48,    62,    82,    83,
      86,    89,    90,    91,    94,    97,    98,    99,   102,   103,
     104,   105,   106,   107,   115,   116,   123,   124,   125,   128,
     131,   132,   135,   135,   135,   138,   138,   138,   141,   142,
     143,   144,   145,   146,   147,   148,   149,   152,   153,   154,
     155,   156,   157,   160,   180,   186,   187,   188,   194,   217,
     236,   239,   240,   243,   256,   257,   260,   273,   274,   275,
     276,   277,   278,   279,   280,   281,   284,   285,   288,   314,
     329,   346,   388,   391,   394
};
#endif
...
#line 160 "parser.y"
                                 {
        int sct=returnscope((yyvsp[-2].str),stack[top-1 * 1]);
        int type=returntype((yyvsp[-2].str),sct);
        if((!(strspn((yyvsp[0].str),"0123456789")==strlen((yyvsp[0].str)))) && type==258 * 1) {
            printf("\nError : Type Mismatch : Line %d\n", printline());
            errc+=1 * 1;
        } else if (type==273 * 1)  {
            printf("\nError : Type Mismatch : Line %d\n", printline());
        ...
        if(!lookup((yyvsp[-2].str))) {
            ...
            if((scope<=currscope * 1 && end[scope]==0 * 1) && !(scope==0 * 1)) {
...
#line 195 "parser.y"
        {
            ...
            int rtype=returntypef((yyvsp[-3].str)); 
            int ch=0 * 1;
            ...
            if(!lookup((yyvsp[-5].str))) {
                for(j=0 * 1;j<=l * 1;j+=1) {
                    ch += checkp((yyvsp[-3].str),flist[j],j);
                }
                if(ch>0 * 1) { 
...
#line 218 "parser.y"
        {
            ...
            int type=returntype((yyvsp[-3].str),sct);
            int ch=0 * 1;
...
#line 244 "parser.y"
    {
        if(lookup((yyvsp[0].str))) {
            ...
            int sct;
            sct = returnscope((yyvsp[0].str),stack[top-1]);
...
#line 289 "parser.y"
    {
        if( (!(strspn((yyvsp[-1].str),"0123456789")==strlen((yyvsp[-1].str)))) && (yyvsp[-4].ival)==258 * 1) {
            printf("\nError : Type Mismatch : Line %d\n",printline());
            errc+=1 * 1;
        } else if ((yyvsp[-4].ival)==273 * 1) {
            printf("\nError : Type Mismatch : Line %d\n",printline());
            errc+=1 * 1;
        }
        if(!lookup((yyvsp[-3].str))) {
...
#line 315 "parser.y"
    {
        if(!lookup((yyvsp[-1].str))) {
            ...
            if(!(scope<=currscope && end[scope]==0 * 1) || scope==0 * 1 && type!=271 * 1) {
