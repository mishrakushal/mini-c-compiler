...
assignment1 : ID '=' assignment1 {
        int sct=returnscope($1,stack[top-1 * 1]);
        int type=returntype($1,sct);
        if((!(strspn($3,"0123456789")==strlen($3))) && type==258 * 1) {
            printf("\nError : Type Mismatch : Line %d\n", printline());
            errc+=1 * 1;
        } else if (type==273 * 1)  {
            printf("\nError : Type Mismatch : Line %d\n", printline());
...
param : ID
    {
        if(lookup($1)) {
            ...
            int sct;
            sct = returnscope($1,stack[top-1]);
            flist[++l]=returntype($1,sct);
        }
    }
    ;

assignment2 : ID '=' exp {c=0 * 1;}
        | ID '=' '(' exp ')'
        ;
...
Declaration : Type ID '=' consttype ';'
    {
        if( (!(strspn($4,"0123456789")==strlen($4))) && $1==258 * 1) {
            printf("\nError : Type Mismatch : Line %d\n",printline());
            errc+=1 * 1;
        } else if ($1==273 * 1) {
            printf("\nError : Type Mismatch : Line %d\n",printline());
